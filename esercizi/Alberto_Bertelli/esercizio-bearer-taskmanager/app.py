...
ACTIVE_TOKENS = {"username": str, "expires": datetime, "created": datetime}  # <-- IMPLEMENTA QUI
...
# }
TASKS = [{"id":int, "username":str,"title":str,"completed":bool,"created_at": datetime,"completed_at": datetime or None}]  
...
def login_required(f):
    ...
    def decorated(*args, **kwargs):
        if "token" not in session:
            flash("effettua il login per accedere alla pagina","error")
            return redirect(url_for("login"))
        username = verify_token(session.get("token"))
        if not username:
            flash("Token scaduto effettua di nuovo la login","error")
...
def get_user_tasks(username, filter_completed=None):
    ...
    results =[t for t in TASKS if t.get("username")==username]
    if filter_completed is None:
        return results
    return [t for t in results if bool(t.get("completed"))==bool(filter_completed)]
...
def get_task_by_id(task_id, username):
    ...
    for task in TASKS:
        if task.get("id")== int(task_id) and task.get("username")== username:
            return task
        return None
...
def update_task(task_id, username, title=None, description=None, completed=None):
    ...
    if not task.get("completed") and new_completed:
