...
job = None
label1 = tk.Label(root, font=("Arial", 16), text="00:00")
label1.pack(pady=20)
...
def update_timer():
    ...
    if timer_attivo:
        ...
        if msecond < 60:
            ...
            var_mseconds.set(0)
            var_second.set(second + 1)
    job = root.after(100, update_timer)
    label1.config(text=f"{second:02d}:{msecond:02d}")
...
def stop():
    ...
    messagebox.showinfo("Cronometro", "Cronometro fermato!")
